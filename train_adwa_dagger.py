"""Train a GRU policy via DAgger on RF-only navigation over ADWA-benchmark buildings.

Near-verbatim port of simple_2D/train_dagger.py, pointed at adwa_env instead
of the procedurally generated toy-room environment: same continuous-heading
regression policy (cos/sin(theta) output, cosine-similarity loss against the
wall-aware BFS teacher direction), same iterative DAgger loop (decaying
teacher/policy rollout mix, replay buffer capped and FIFO-evicted, LR/weight
decay), same resumable per-round checkpointing. The only real difference is
the map pool: ADWANavigationEnv splits real building floorplans train/holdout
(see adwa_env.HOLDOUT_BUILDINGS) instead of splitting procedurally generated
maps, so holdout accuracy here measures generalisation across genuinely
different real floorplans.

Example: poetry run python train_adwa_dagger.py --rounds 10 --episodes-per-round 150
"""
...
@torch.no_grad()
def evaluate(policy: RecurrentPolicy, env: ADWANavigationEnv, device: torch.device,
             episodes: int = 60, split: str = "train") -> tuple[float, float]:
    """Deterministic (greedy) success rate and mean episode reward.

    split="holdout" draws only from buildings never used during
    split="train" rollouts, so it measures generalisation to unseen real
    floorplans rather than just an unlucky random draw within one building.
    """
    policy.eval()
...
def collect_round(policy: RecurrentPolicy, env: ADWANavigationEnv, episodes: int,
...
def trim_replay(demonstrations: list, cap: int) -> list:
    ...
    keeps round time roughly constant as training progresses and keeps the
    dataset focused on states the current policy actually visits."""
...
def main():
    ...
    parser.add_argument("--weight-decay", type=float, default=1e-5,
                        help="L2 regularisation, to discourage memorising the training-building set.")
    ...
    parser.add_argument("--resume", action="store_true",
                        help="Continue from checkpoints/adwa_dagger_resume.pt if present, instead of round 1.")
    ...
    resume_path = Path("checkpoints/adwa_dagger_resume.pt")
    ...
    for round_idx in range(start_round, args.rounds + 1):
        ...
        eval_env = ADWANavigationEnv(seed=args.seed + round_idx)
        ...
        if holdout_success >= best_holdout_success:
            ...
            torch.save({"model": policy.state_dict(), "observation_size": env.observation_size,
                       "env_config": asdict(env.config)}, "checkpoints/adwa_gru_dagger.pt")
    ...
    print(f"Best holdout-building success={best_holdout_success:.1%}; saved checkpoints/adwa_gru_dagger.pt")
